...
export default function Carousel({
  ...
  // автоплей
  useEffect(() => {
    if (!autoPlay || len <= 1 || isPaused) return; // ADDED: уважать паузу
    const id = setInterval(() => {
      if (isPausedRef.current) return; // ADDED: двойная защита
      isAutoScrollRef.current = true; // ADDED: помечаем автоскролл
      ref.current?.scrollToOffset({
  ...
  }, [autoPlay, intervalMs, len, cardW, gap, absIndex, isPaused]); // ADDED: isPaused в deps
  ...
  // ADDED: при старте ручного свайпа — ставим паузу
  ...
  function onMomentumEnd(e: NativeSyntheticEvent<NativeScrollEvent>) {
    ...
    // ADDED: если это был автоскролл — сбрасываем флаг и продолжаем жить как раньше
    if (isAutoScrollRef.current) {
      isAutoScrollRef.current = false;
      return;
    }
    // ADDED: если это был ручной свайп — планируем возобновление через idle-таймаут
  ...
  return (
    <View>
      <FlatList
        ...
        bounces={false}
        onScrollBeginDrag={onScrollBeginDrag} // ADDED
